...
(deftest collections-are-broken
  (are
       [expected-alt-form test-form]
    (= expected-alt-form (:alt (kibit/check-expr test-form)))
    '(update-in coll [?key] f) '(assoc coll k (f (k coll)))
    '(update-in coll [?key] f) '(assoc coll k (f (coll k)))
    '(update-in coll [?key] f) '(assoc coll k (f (get coll k)))
    '(update-in coll [?key] f a b c) '(assoc coll k (f (k coll) a b c))
    '(update-in coll [?key] f a b c) '(assoc coll k (f (coll k) a b c))
    '(update-in coll [?key] f a b c) '(assoc coll k (f (get coll k) a b c)))
  )
